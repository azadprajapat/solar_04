    ...
    ProcessImage({this.imagepaths,this.camera_modal,this.size});
     ...
     Process(context,callback) async{
          ...
          Immage.Image img = await Immage.decodeImage(file);
           for (int i = 0; i < img.width; i++) {
            for (int j = 0; j < img.height; j++) {
              ...
              var E = await (imagepaths[t].elevation - atan(y/(camera_modal.focal_length))*180/pi);
               if(E>90){
                E=90.0;
              }
              if(E<0){
      ...
      //matrix[A][E]==0 if current angle(A,E) lie in skyzone and 1 if lies in nonsky zone
              ...
              else if(Color(pixel).blue>=Color(pixel).red&&Color(pixel).blue>=Color(pixel).green&&Color(pixel).blue>=75 &&i*j<(img.width*img.height)/2
              ){
                 matrix[A][E]=0;
              }
              else{
